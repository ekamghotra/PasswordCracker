...
public class Password {
  ...
  /**
   * Creates a password objects with all fields (used for comparisons) at their
   * lower bound for the minimum value. Used ONLY for implementation of front facing
   * PasswordStorage.isValidBST() method.
   *
  ...
  public static Password getMinPassword() {
    ...
    min.hashedPassword = MIN_SHA1_HASH; // 0 is smallest hexidecimal value, all hash strings are 40 long.
    return min;
  }
  ...
  public static Password getMaxPassword() {
    ...
    max.strengthRating = (float) (4 * 1.75 + MAX_LENGTH + 0.1); // 0.1 over the max possible strengthRating calculated
    max.hashedPassword = MAX_SHA1_HASH;// f is largest hexidecimal value, all hash strings are 40 long
    return max;
  }
  ...
  private void computeAndSetHashedPassword() {
    ...
    } catch (NoSuchAlgorithmException e) { // if "SHA-1" isn't a supported algorithm, catch the exception
  ...
  public String toString() {
    return this.password + "(" + this.hashedPassword + "): " + this.occurrences + "[" + this.strengthRating + "]";
  }
  ...
  public int compareTo(Password other, Attribute a) {
    switch (a) {
      case OCCURRENCE:
        return Integer.compare(occurrences, other.occurrences);
      case STRENGTH_RATING:
        return Float.compare(strengthRating, other.strengthRating);
